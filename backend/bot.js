...
const log = require("./logger"); // Логгер
...
async function processBatchRewards() {
  ...
  for (const [userId, userData] of activeUsers) {
    ...
    const rewardAmount = CONFIG.BASE_REWARD * timeInVoiceSeconds * rewardMultiplier;
    ...
    try {
      ...
      log.error(`Ошибка начисления награды для пользователя ${userId}`, { error });
    }
...
client.on("voiceStateUpdate", (oldState, newState) => {
  ...
  if (newChannelId && newChannelId !== CONFIG.AFK_CHANNEL_ID) {
    ...
    activeUsers.set(userId, {
      ...
      joinTime: activeUsers.has(userId) ? activeUsers.get(userId).joinTime : Date.now(),
    });
    logEvent("Пользователь зашел/переключился в канал", { userId, newChannelId });
